...
- (id)initWithFrame1:(CGRect)frame{
    ...
    if (self) {
        ...
        //获取到关联单位和所有单位
        [[NSNotificationCenter defaultCenter] removeObserver:self name:@"getUnitClass" object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getUnitClass:) name:@"getUnitClass" object:nil];
        //最新、推荐文章数量
        [[NSNotificationCenter defaultCenter] removeObserver:self name:@"GetSectionMessage" object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(GetSectionMessage:) name:@"GetSectionMessage" object:nil];
        ...
        [self.mCollectionV_unit registerClass:[ShareCollectionViewCell class] forCellWithReuseIdentifier:CellID];
//        [self.mCollectionV_unit registerClass:[Forward_section class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:DetailedViewControllerID];
//        self.mCollectionV_unit.scrollEnabled = NO;
        //设置本身大小和内容大小
...
-(void)getIdentity:(NSNotification *)noti{
    ...
    //获取数量
    [[ShareHttp getInstance] shareHttpGetUnitSectionMessagesWith:@"1" AcdID:[NSString stringWithFormat:@"%d",[dm getInstance].uType]];
    //获取最新、推荐未读数量
    [[ShareHttp getInstance] shareHttpGetSectionMessageWith:@"_2" TopFlags:@"1" AccID:[NSString stringWithFormat:@"%d",[dm getInstance].uType]];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([userInfoModel.UnitType intValue] ==1||[userInfoModel.UnitType intValue] ==0){//当为教育局或最新更新、推荐
        ...
        if (cell == nil) {
            cell = [[TopArthListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellWithIdentifier];
            ...
            [self.mTableV_detail registerNib:n forCellReuseIdentifier:CellWithIdentifier];
        }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([userInfoModel.UnitType intValue] ==1||[userInfoModel.UnitType intValue] ==0){//当为教育局或最新更新、推荐，进入文章详情
        ...
        if (node.type == 0) {//判断为最大层级
            if (node.readflag == 0) {//本单位分享
                ...
                if (node.isExpanded) {//根据现在是否打开状态，判断是否发生请求
                    if (node.readflag == 1) {//关联的班级
                        [[ShareHttp getInstance] shareHttpGetMyUserClassWith:[dm getInstance].jiaoBaoHao UID:userInfoModel.UnitID Section:@"1"];
            ...
            TreeView_Level0_Model *nodeData = node.nodeData;
            TreeView_node *node1 = [mArr_display objectAtIndex:indexPath.row];
            ArthLIstViewController *list = [[ArthLIstViewController alloc] init];
            list.mInt_section = 0;
            list.mInt_class = node.readflag;
            D("list.mInt_class0--===%d",node1.readflag);
...
//    Forward_section *view = [collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:DetailedViewControllerID forIndexPath:indexPath];
//    [view initWithFrame1:CGRectMake(0, 0, 0, 0)];
//    return view;
//}
//设置每组的cell的边界
