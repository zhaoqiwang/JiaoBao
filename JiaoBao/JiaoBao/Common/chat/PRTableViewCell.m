...
@interface PRUtilityButtonView : UIView

@property (nonatomic, strong) NSArray *utilityButtons;
@property (nonatomic) CGFloat utilityButtonWidth;
@property (nonatomic, weak) PRTableViewCell *parentCell;
@property (nonatomic) SEL utilityButtonSelector;

- (id)initWithUtilityButtons:(NSArray *)utilityButtons parentCell:(PRTableViewCell *)parentCell utilityButtonSelector:(SEL)utilityButtonSelector;

- (id)initWithFrame:(CGRect)frame utilityButtons:(NSArray *)utilityButtons parentCell:(PRTableViewCell *)parentCell utilityButtonSelector:(SEL)utilityButtonSelector;

@end

@implementation PRUtilityButtonView
...
@interface PRTableViewCell () <UIScrollViewDelegate> {
    PRCellState _cellState; // The state of the cell within the scroll view, can be left, right or middle
}
...
@property (nonatomic, strong) PRUtilityButtonView *scrollViewButtonViewLeft;
@property (nonatomic, strong) PRUtilityButtonView *scrollViewButtonViewRight;

// Used for row height and selection
@property (nonatomic, weak) UITableView *containingTableView;

//@property (nonatomic, strong) UIImageView *unreadBackgroundImageView;
...
- (void)initializer {
    ...
    PRUtilityButtonView *scrollViewButtonViewLeft = [[PRUtilityButtonView alloc] initWithUtilityButtons:_leftUtilityButtons parentCell:self utilityButtonSelector:@selector(leftUtilityButtonHandler:)];
    ...
    PRUtilityButtonView *scrollViewButtonViewRight = [[PRUtilityButtonView alloc] initWithUtilityButtons:_rightUtilityButtons parentCell:self utilityButtonSelector:@selector(rightUtilityButtonHandler:)];
...
//    unreadImageView.image = [UIImage imageNamed:@"homepage_newmessage"];
//    self.unreadBackgroundImageView = unreadImageView;
    
    UILabel *unreadLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 18, 18)];
    unreadLabel.textAlignment = NSTextAlignmentCenter;
    unreadLabel.font = [UIFont systemFontOfSize:12.0];
    unreadLabel.textColor = [UIColor whiteColor];
    unreadLabel.layer.cornerRadius = 9;
    //unreadLabel.layer.borderWidth = 1;
    //unreadLabel.layer.borderColor = RedColor.CGColor;
    unreadLabel.clipsToBounds = YES;
    unreadLabel.backgroundColor = RedColor;
    [self.cellScrollView addSubview:unreadLabel];
    self.unreadLabel = unreadLabel;
    
    UILabel *timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 100, 21)];
    timeLabel.textAlignment = NSTextAlignmentRight;
    timeLabel.font = [UIFont systemFontOfSize:12.0];
    timeLabel.textColor = GrayColorBB;
    ...
    self.detailTextLabel.textColor = GrayColor99;
}
...
- (void)layoutSubviews {
    ...
    _height = self.contentView.bounds.size.height;
    ...
    self.imageView.frame = CGRectMake(15, (_height-50)/2.0f, 50, 50);
    
    if ([self.textLabel.text length] > 0)
    {
        [self.textLabel setOriginX:80];
        
        if ([self.detailTextLabel.text length] > 0)
        {
            [self.textLabel setOriginY:15];
            [self.detailTextLabel setOriginY:40];
        }
    }
    if ([self.detailTextLabel.text length] > 0)
    {
        [self.detailTextLabel setOriginX:80];
    }
    ...
    if (_unreadcount > 0)
    {
        ...
        CGRect unreadLabelRect = self.unreadLabel.frame;
        self.unreadLabel.center = CGPointMake(imageViewRect.origin.x + imageViewRect.size.width/*-unreadLabelRect.size.width/2.0f*/, imageViewRect.origin.y + unreadLabelRect.size.height/2.0f);
        self.unreadLabel.text = [NSString stringWithFormat:@"%d", _unreadcount];
    }
    else
    {
    ...
    if ([self.timeString length] > 0)
    {
        ...
        self.timeLabel.frame = CGRectMake(width-15-100, 12, 100, 21);
    }
    else
    {
