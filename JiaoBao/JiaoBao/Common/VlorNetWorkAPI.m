...
@implementation NSObject (VlorNetWorkAPI)
...
{
    [self requestToUrl:url method:@"GET" useCache:NO params:params headersParams:headersParams complete:complete];
}
...
{
    [self requestToUrl:url method:@"GET" useCache:YES params:params headersParams:headersParams complete:complete];
}
...
{
    [self requestToUrl:url method:@"POST" useCache:NO params:params headersParams:headersParams complete:complete];

}
- (NSMutableURLRequest *)requestWithUrl:(NSString *)url method:(NSString *)method useCache:(BOOL)useCache params:(NSDictionary *)params
{
    params = [[VlorNetWorkAPI getRequestBodyWithParams:params] copy];
...
- (void)localCacheToUrl:(NSString *)url params:(NSDictionary *)params headersParams:(NSDictionary *)headersParams complete:(void (^)(BOOL successed, NSDictionary *result))complete
{
    ...
    if (cachedResponse != nil && [[cachedResponse data] length] > 0) {
        ...
        [self getCacheToUrl:url params:params headersParams:headersParams complete:complete];
    }
}
- (void)requestToUrl:(NSString *)url method:(NSString *)method useCache:(BOOL)useCache
              params:(NSDictionary *)params headersParams:(NSDictionary *)headersParams complete:(void (^)(BOOL successed, NSDictionary *result))complete
{
    ...
    NSArray *heardersKey = [headersParams allKeys];
    NSArray *heardersValue = [headersParams allValues];
    for (int i = 0; i < [heardersKey count]; i++) {
        [operationManager.requestSerializer setValue:[heardersValue objectAtIndex:i] forHTTPHeaderField:[heardersKey objectAtIndex:i]];
    }
